...
const searchBar = document.querySelector(".inputs");
...
totalLength.textContent = `Currently, we have ${length} countries`;

const countries = (country) => {
  containing.innerHTML = "";
  if (searchBar.value != "") {
    const length = country.map((el) => el.name).length;
    containing.textContent = `${length} countries satisfied the search criteria`;
  }
  //countryContent
  content.innerHTML = "";
  country.forEach((el) => {
    let html = `<div class="countries">
            <div class="img">
                <img class="imgCountry" src="${el.flag}" alt="" />
                <p class="nameCountry">${el.name}</p>`;
    if (el.capital !== undefined) {
      html += `<p class="capitalCountry">Capital: ${el.capital}</p>`;
    }
    html += `<p class="languageCountry">Languages: ${el.languages}</p>
                <p class="populationCountry">Population: ${el.population}</p>
            </div>
        </div>`;
...
const search = () => {
  ...
  const search = country.filter((el) =>
    el.name.toLowerCase().includes(searchTxt)
  );
  countries(search);
};

const sortingByName = () => {
  const sorts = country.sort((a, b) => b.name.localeCompare(a.name));
  search(sorts);
};
const sortAllName = () => {
  const sorting = country.sort((a, b) => b.name.localeCompare(a.name));
  countries(sorting);
};

const sortingByCapital = () => {
  const sorts = country.sort((a, b) => {
    if (a.capital !== undefined && b.capital !== undefined) {
      const as = a.capital.toLowerCase();
      const bs = b.capital.toLowerCase();
      return bs.localeCompare(as);
    }
  });
  search(sorts);
};
const sortAllCapital = () => {
...
const sortingByPopulation = () => {
  const sorts = country.sort((a, b) => a.population - b.population);
  search(sorts);
};
const sortAllPopulation = () => {
  const sorting = country.sort((a, b) => a.population - b.population);
  countries(sorting);
};

sortPopulation.addEventListener("click", () => {
  searchBar.value == "" ? sortAllPopulation() : sortingByPopulation();
});

sortCapital.addEventListener("click", () => {
  searchBar.value == "" ? sortAllCapital() : sortingByCapital();
});

sortName.addEventListener("click", () => {
  searchBar.value == "" ? sortAllName() : sortingByName();
});
...
const fetchLanguage = () => {
  const viewPop = document.querySelector(".country-container1");
  const viewLang = document.querySelector(".country-container2");

  viewLang.style.display = "block";
...
const popFill = () => {
  ...
  viewPop.innerHTML = "";
  const searchTxt = searchBar.value.toLowerCase();
  const sorted = country.sort((a, b) => b.population - a.population);
  const arr = sorted.map((el) => [el.name, el.population]);
  const fill = arr.filter((el) => el[0].toLowerCase().includes(searchTxt));
  console.log(fill);
